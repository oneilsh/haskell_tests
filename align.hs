...
score_aln a b 
	| length a == 1 && length b == 1 = score_pair a b
	| otherwise = score_aln (tail a) (tail b) + score_pair [(head a)] [(head b)] -- we always work with strings :)
...
global_aln a b
	...
	| otherwise = let {b_a = [head a];
	                   ...
	                   left = global_aln_mem s_a (b_b ++ s_b);
	                   center = global_aln_mem s_a s_b;
	                   right = global_aln_mem (b_a ++ s_a) s_b;
